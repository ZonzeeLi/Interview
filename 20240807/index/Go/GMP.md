...
2. G 运行完成后：M1 上的 G1 运行完成后（调用`goexit()`函数），M1 上运行的 Goroutine 会切换为 G0，G0 负责调度协程的切换（运行`schedule()`函数），从 M1 上 P 的本地运行队列获取 G2 去执行（函数`execute()`）；

> 注意，这里 G0 室程序启动时的线程 M（也叫 M0）的系统栈表示的 G 结构体，负责 M 上 G 的调度

![GMP调度过程2](../image/GMP调度过程2.png)
...
> 注意，这里的图片和描述均有误，实际上是会指向 runnext 为这个新建的 G，然后当前指向的 runnext 和前一半进入全局队列 

![GMP调度过程3](../image/GMP调度过程3.png)
...
![GMP调度过程4](../image/GMP调度过程4.png)

5. 任务窃取机制：自旋状态的 M 会寻找可运行的 G，如果全局队列为空，则会从其他 P 偷取 G 来执行，个数是其他 P 运行队列的一半；

![GMP调度过程5](../image/GMP调度过程5.png)

6. G 发生系统调用时：如果 G 发生系统调度进入阻塞，其所在的 M 也会阻塞，因为会进入内核状态等待系统资源，和 M 绑定的 P 会寻找空闲的 M 执行，这是为了提高效率，不能让 P 本地队列的 G 因所在 M 进入阻塞状态而无法执行；需说明的是，如果是 M 上的 G 进入 Channel 阻塞，则该 M 不会一起进入阻塞，因为 Channel 数据传输涉及内存拷贝，不涉及系统资源等待；

![GMP调度过程6](../image/GMP调度过程6.png)

7. G 退出系统调用时：如果刚才进入系统调用的 G2 解除了阻塞，其所在的 M1 会寻找 P 去执行，优先找原来的 P，发现没有找到，则其上的 G2 会进入全局队列，等其他 M 获取执行，M1 进入空闲队列；

![GMP调度过程7](../image/GMP调度过程7.png)
