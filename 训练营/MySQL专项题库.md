...
读未提交：会发生发生脏读、幻读、不可重复读问题。
读提交：不会发生脏读，会发生幻读、不可重复读。
可重复读：不会发生脏读和不可重复读，会发生幻读。
串行化：不会发生这些问题。
...
MySQL主机的bin log刷盘，从机建立一个线程监控主机的log dump，将bin log写入到从机的relay log中，然后会有另一个线程来读取relay log。
